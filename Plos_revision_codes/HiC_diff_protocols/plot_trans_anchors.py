...
for b_idx in range(2):
    ...
    for e_idx1, e_idx2 in itertools.combinations(list(range(len(labels))), 2):
        ...
        # trans-anchors
        anchor1_list = e1_info['trans']
        anchor2_list = e2_info['trans']
        ...
        trans_dist_mat = np.ones((n1, n2))*(math.inf)
        ...
        for a1_idx, anchor1 in enumerate(anchor1_list):
            ...
            for a2_idx, anchor2 in enumerate(anchor2_list):
                ...
                if a1_chrom1 != a2_chrom1 and a1_chrom1 != a2_chrom2:
                    continue

                if a1_chrom2 != a2_chrom1 and a1_chrom2 != a2_chrom2:
                    continue

                if a1_chrom1 == a2_chrom2:
                    a2_bin1 = anchor2[3]
                    a2_bin2 = anchor2[1]
                else:
                    a2_bin1 = anchor2[1]
                ...
                trans_dist_mat[a1_idx, a2_idx] = ddist

        minn_1 = np.min(trans_dist_mat, axis=0)
        minn_2 = np.min(trans_dist_mat, axis=1)

        minnn = list(minn_1) + list(minn_2)

        all_trans_dists = np.array(minnn)

        uni, counts = np.unique(all_trans_dists, return_counts=True)
    ...
    plt.savefig('trans_anchor_distances_ALL_scaled_dim'+str(dim)+'_bidx_'+str(b_idx)+'.pdf', dpi=600)
    #plt.show()
