...
def search_edges(n_elements, bin1, bin2, pixels, weights, g_dist, edges\
    ...
    for i in range(n_elements):
        ...
        for alpha_idx in range(len(alpha_vals)):
            edges[count, 3+alpha_idx] = (bal_val)**alpha_vals[alpha_idx]
        count += 1
...
for idx, exp in enumerate(exps):
    ...
    # Assuming that entries with bin dist <= 10 are at most half of total entries
    edges = np.ones((int(n_elements/2), 3+len(alpha_vals)))
    ...
    filename =\
        '../cooler_results/edge_files/'+labels[idx]+'_edges_upto_bindist'+str(g_thresh)+'_diff_scaling.npy'
